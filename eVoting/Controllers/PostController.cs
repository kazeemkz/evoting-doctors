...
namespace eVoting.Controllers
{
    ...
    public class PostController : Controller
    {
        ...
        public ActionResult Voted(string PRESIDENT, string VICE_PRESIDENT, string GENERAL_SECRETARY,
            ...
            string SOCIAL_DIRECTOR, string WELFARE_OFFICER, string SPORT_DIRECTOR, string ASST_SPORT_DIRECTOR, string SOCIAL_OFFICER_MALE,
           string SOCIAL_OFFICER_FEMALE, string SPORTS_OFFICER_MALE, string SPORTS_OFFICER_FEMALE, string theKey)
        {
            ...
            if (theCounter > 0)
            {
                ...
                int spof = 0;
                if (!(string.IsNullOrEmpty(SPORTS_OFFICER_FEMALE)))
                {
                    string[] c = SPORTS_OFFICER_FEMALE.Split(':');
                    if (c.Count() == 1)
                    {
                        spof = Convert.ToInt32(SPORTS_OFFICER_FEMALE);
                        Participant Par = context.Participants.Find(spof);
                        Par.Vote = Par.Vote + 1;
                    }
                    if (c.Count() == 2)
                    {
                        spof = Convert.ToInt32(c[0]);
                        Participant Par = context.Participants.Find(spof);
                        if (c[1] == "YES")
                        {
                            Par.Yes = Par.Yes + 1;
                ...
                int spom = 0;
                if (!(string.IsNullOrEmpty(SPORTS_OFFICER_MALE)))
                {
                    string[] c = SPORTS_OFFICER_MALE.Split(':');
                    if (c.Count() == 1)
                    {
                        spom = Convert.ToInt32(SPORTS_OFFICER_MALE);
                        Participant Par = context.Participants.Find(spom);
                        Par.Vote = Par.Vote + 1;
                    }
                    if (c.Count() == 2)
                    {
                        spom = Convert.ToInt32(c[0]);
                        Participant Par = context.Participants.Find(spom);
                        if (c[1] == "YES")
                        {
                            Par.Yes = Par.Yes + 1;
                ...
                int sof = 0;
                if (!(string.IsNullOrEmpty(SOCIAL_OFFICER_FEMALE)))
                {
                    string[] c = SOCIAL_OFFICER_FEMALE.Split(':');
                    if (c.Count() == 1)
                    {
                        sof = Convert.ToInt32(SOCIAL_OFFICER_FEMALE);
                        Participant Par = context.Participants.Find(sof);
                        Par.Vote = Par.Vote + 1;
                    }
                    if (c.Count() == 2)
                    {
                        sof = Convert.ToInt32(c[0]);
                        Participant Par = context.Participants.Find(sof);
                        if (c[1] == "YES")
                        {
                            Par.Yes = Par.Yes + 1;
                ...
                int som = 0;
                if (!(string.IsNullOrEmpty(SOCIAL_OFFICER_MALE)))
                {
                    string[] c = SOCIAL_OFFICER_MALE.Split(':');
                    if (c.Count() == 1)
                    {
                        som = Convert.ToInt32(SOCIAL_OFFICER_MALE);
                        Participant Par = context.Participants.Find(som);
                        Par.Vote = Par.Vote + 1;
                    }
                    if (c.Count() == 2)
                    {
                        som = Convert.ToInt32(c[0]);
                        Participant Par = context.Participants.Find(som);
                        if (c[1] == "YES")
                        {
                            Par.Yes = Par.Yes + 1;
        ...
        public FileStreamResult PrintPassword()
        {
            ...
            HashSet<string> theDepartments = new HashSet<string>();
            ...
            //theDepartments.Add("200level Biodemical");
            ...
            theDepartments.Add("ACCOUNTS");
            theDepartments.Add("ANNALS OF IBADAN");
            theDepartments.Add("ACCIDENT & EMERGENCY");
            theDepartments.Add("ANAESTHESIA");
            theDepartments.Add("CHEMICAL PATHOLOGY");
            theDepartments.Add("CLINICAL PHARM");
            theDepartments.Add("COMMUNITY MEDICINE");
            theDepartments.Add("CONSULTANT");
            theDepartments.Add("DENTISTRY");
            theDepartments.Add("GENERAL OUTPATIENTS");
            theDepartments.Add("GERIATRICS CENTRE");
            ...
            theDepartments.Add("ORTHORINOLARYNGOLOGY");
            ...
            theDepartments.Add("STAFF MEDICAL SERVICES");
            theDepartments.Add("ORTHORPAEDIC & TRAUMA");
         ...
         //  theDepartments.Order;
            ...
            foreach (string d in theDepartments)
            {
                ...
                foreach (Voter v in theVoters)
                {
                    ...
                    string staffID = v.FirstName + " Staff No- " + v.Matric;
